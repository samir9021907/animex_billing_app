  ...
  return (
    <>
      ...
      <header className="bg-animex-blue-900 text-white shadow-xl sticky top-0 z-40">
        ...
        {/* Mobile Horizontal Menu Bar (Always visible on mobile screens) */}
        <div className="lg:hidden flex items-center gap-2 px-3 py-2 overflow-x-auto no-scrollbar bg-[#091E36] border-t border-white/10 shadow-inner">
          <button
            onClick={() => handleTabClick('dashboard')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex-shrink-0 cursor-pointer ${
              activeTab === 'dashboard'
          ...
          >
            ...
            <span>{t('nav.history')}</span>
            <span className="bg-white/20 text-white px-1.5 py-0.2 rounded text-[10px]">{invoicesCount}</span>
          </button>
          <button
            onClick={() => handleTabClick('stores')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex-shrink-0 cursor-pointer ${
