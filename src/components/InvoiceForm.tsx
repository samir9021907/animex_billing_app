...
const UnitSelector: React.FC<UnitSelectorProps> = ({ value, onChange, className }) => {
  ...
  const options = Array.from(new Set([value, ...UNIT_OPTIONS])).filter(Boolean);
  ...
  return (
    <div className={`relative ${className || ''}`} ref={dropdownRef}>
      <button
        ...
        className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg p-2 font-bold text-slate-900 dark:text-white flex items-center justify-between text-xs sm:text-sm shadow-sm cursor-pointer hover:border-slate-400 focus:outline-none focus:ring-2 focus:ring-animex-blue-500"
      >
        <span className="truncate">{value || 'Select'}</span>
        <ChevronDown className={`w-3.5 h-3.5 text-slate-400 shrink-0 transition-transform duration-200 ${isOpen ? 'rotate-180 text-animex-blue-600' : ''}`} />
      </button>

      {isOpen && (
        <div className="absolute z-50 left-0 right-0 mt-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl shadow-2xl max-h-36 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800 touch-pan-y overscroll-contain">
          {options.map((opt) => (
            <button
              key={opt}
              type="button"
              ...
              className={`w-full text-left px-3 py-2.5 text-xs font-bold transition-colors flex items-center justify-between cursor-pointer ${
                value === opt
                  ? 'bg-animex-blue-50 dark:bg-animex-blue-950/60 text-animex-blue-600 dark:text-sky-400 font-black'
                  : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 active:bg-slate-200'
              }`}
            >
              <span>{opt}</span>
              {value === opt && <Check className="w-3.5 h-3.5 text-animex-blue-600 dark:text-sky-400" />}
            </button>
          ))}
        </div>
      )}
